...
params:
  input_scores_folder: "./Input_Scoreboard_Data"
...
```{r numeric months to names, include=FALSE}
...
# Set name of subfolder with results
input_scores_folder <- params$input_scores_folder
...
# Get all files beginning with "results-"
listFiles <- list.files(path = input_scores_folder, pattern = results_string)
...
for (i in 1:length(listFiles)){
  ...
  res <- parseTableV2(
    table_name = paste(input_scores_folder, listFiles[i], sep = '/'),
    tables_w_assists_string = results_string
    )
